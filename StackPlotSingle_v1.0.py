...
f, ax1a = plt.subplots(1, 1, dpi=300, figsize=(6.5, 3.9), sharey=False)
...
# Comment out what is not needed for different plots
...
while i < 1:
    ...
    df = pd.read_csv(inputFilePath + '/' + fileNameList[i] + ".csv", index_col=['siteID'])
    axM1[i].stackplot(df.wA, df.bL, df.bH, df.nF, df.nS, df.aR, df.aNR, labels=labels, colors=colors)
    axM1[i].set_title(fileNameList[i])
...
ax1a.legend(loc='upper center', bbox_to_anchor=(0.5, -.15),
          fancybox=False, shadow=False, ncol=6)
          
plt.tight_layout()

plt.savefig(outputFilePath + '/' + 'BMRSA1617.png', bbox_inches='tight')
# plt.savefig(outputFilePath + '/' + 'EC1617.png', bbox_inches='tight')
# plt.savefig(outputFilePath + '/' + 'DO1617.png', bbox_inches='tight')
